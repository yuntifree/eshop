...
class Op_EweiShopV2Page extends WebPage 
{
	...
	public function changeprice() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			m('notice')->sendMessage($item['openid'], array('title' => '订单价格', 'ordersn' => $item['ordersn'], 'olddata' => $item['price'], 'data' => $orderprice, 'type' => 1), 'orderstatus');
			show_json(1);
		}
	...
	public function sendcancel() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			pdo_update('ewei_shop_order', array('status' => 1, 'sendtime' => 0, 'remarksend' => $remark), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			if ($item['paytype'] == 3) 
			{
				m('order')->setStocksAndCredits($item['id'], 2);
			}
	...
	public function changeaddress() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				m('notice')->sendMessage($item['openid'], array('title' => '订单收货地址', 'ordersn' => $item['ordersn'], 'olddata' => $oldaddress['address'], 'data' => $province . ' ' . $city . ' ' . $area . ' ' . $address, 'type' => 0), 'orderstatus');
				show_json(1);
			}
		}
		include $this->template();
