...
class Finance_EweiShopV2Model 
{
	...
	public function AlipayRefund($params, $config) 
	{
		...
		$parameter = array('service' => 'refund_fastpay_by_platform_pwd', 'partner' => $config['partner'], '_input_charset' => 'UTF-8', 'notify_url' => (isset($params['notify_url']) ? $params['notify_url'] : ''), 'seller_user_id' => $config['partner'], 'seller_email' => $config['account'], 'refund_date' => date('Y-m-d H:i:s'), 'batch_no' => date('YmdHis'), 'batch_num' => '1', 'detail_data' => $params['trade_no'] . '^' . $params['refund_price'] . '^' . $params['refund_reason']);
