...
class DiypageModel extends PluginModel 
{
			...
			if (!(empty($page['data']['items'])) && is_array($page['data']['items'])) 
			{
				foreach ($page['data']['items'] as $itemid => &$item ) 
				{
					if ($item['id'] == 'goods') 
					{
						...
						if ($item['params']['goodsdata'] == '0') 
						{
							if (!(empty($item['data'])) && is_array($item['data'])) 
							{
								...
								if (!(empty($goodsids)) && is_array($goodsids)) 
								{
									...
									{
										$goods = pdo_fetchall('select id, title, thumb, productprice, minprice, total, showlevels, showgroups, bargain, merchid, sales from ' . tablename('ewei_shop_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
									}
										...
										foreach ($goodsids as $goodsid ) 
										{
											foreach ($goods as $index => $good ) 
											{
												if ($good['id'] == $goodsid) 
												{
													...
													$childid = 'C' . $childid;
													$item['data'][$childid] = array('thumb' => $good['thumb'], 'title' => $good['title'], 'price' => $good['minprice'], 'gid' => $good['id'], 'total' => $good['total'], 'bargain' => $good['bargain'], 'productprice' => $good['productprice'], 'credit' => $good['credit'], 'ctype' => $good['type'], 'gtype' => $good['goodstype'], 'sales' => $good['sales']);
												}
											}
										}
									}
						...
						else if (($item['params']['goodsdata'] == '2') && empty($item['params']['goodstype'])) 
						{
					...
					else if (($item['id'] == 'merchgroup') && p('merch')) 
					{
						...
						else if ($item['params']['merchdata'] == '3') 
						{
							...
							if (!(empty($merchs)) && !(empty($merchids)) && is_array($merchids)) 
							{
								$item['data'] = array();
								foreach ($merchs as $index => $merch ) 
								{
	...
	public function savePage($id, $data, $update = true) 
	{
		...
		if ($update) 
		{
			...
			if ($savetype == 'update') 
			{
				...
				else if ((1 < $item['type']) && ($item['type'] < 99)) 
				{
			...
			if ($savetype == 'insert') 
			{
				...
				else if ((1 < $diypage['type']) && ($diypage['type'] < 99)) 
				{
					$pagetype = 'sys';
				}
				else if ($diypage['type'] == 99) 
				{
					$pagetype = 'mod';
				}
	...
	public function saveTemp($temp) 
	{
		...
		else if ((1 < $temp['type']) && ($temp['type'] < 99)) 
		{
	...
	public function verify($do, $pagetype) 
	{
		...
		else if ($do == 'edit') 
		{
			...
			else if (($pagetype == 'sys') && (($page['type'] < 2) || (99 <= $page['type']))) 
			{
				$type = $this->getPageType($page['type']);
				$url = webUrl('diypage/page') . '.' . $type['pagetype'] . '.edit&id=' . $id;
				header('location: ' . $url);
			...
			else if (($pagetype == 'mod') && ($page['type'] != 99)) 
			{
				$type = $this->getPageType($page['type']);
				$url = webUrl('diypage/page') . '.' . $type['pagetype'] . '.edit&id=' . $id;
				header('location: ' . $url);
	...
	{
		$pagetype = array( 1 => array('name' => '自定义', 'pagetype' => 'diy', 'class' => ''), 2 => array('name' => '商城首页', 'pagetype' => 'sys', 'class' => 'success'), 3 => array('name' => '会员中心', 'pagetype' => 'sys', 'class' => 'primary'), 4 => array('name' => '分销中心', 'pagetype' => 'sys', 'class' => 'warning'), 5 => array('name' => '商品详情页', 'pagetype' => 'sys', 'class' => 'danger'), 6 => array('name' => '积分商城', 'pagetype' => 'sys', 'class' => 'info'), 7 => array('name' => '整点秒杀', 'pagetype' => 'sys', 'class' => 'danger'), 99 => array('name' => '公用模块', 'pagetype' => 'mod', 'class' => '') );
		if (!(empty($type))) 
		{
	...
	public function detailPage($pageid = 0) 
	{
		...
		foreach ($pageitems as $itemid => $pageitem ) 
		{
			...
			else if ($pageitem['id'] == 'detail_seckill') 
			{
		...
		if (!(empty($detail_navbar))) 
		{
			...
			if (intval($detail_navbar['params']['hidelike']) < 1) 
			{
				...
				else if ($detail_navbar['params']['hidelike'] == -3) 
				{
			...
			if (intval($detail_navbar['params']['hideshop']) < 1) 
			{
				...
				else if ($detail_navbar['params']['hideshop'] == -3) 
				{
			...
			if (intval($detail_navbar['params']['hidecart']) < 1) 
			{
				...
				else if ($detail_navbar['params']['hidecart'] == -3) 
				{
