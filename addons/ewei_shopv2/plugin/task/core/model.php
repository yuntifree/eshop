		...
		else if ($join_info['task_type'] == 1) 
		{
			...
			if (!(empty($rec_reward))) 
			{
				...
				if (isset($rec_reward['goods'][$goods_id]) && !(empty($rec_reward['goods'][$goods_id]))) 
				{
					...
					$rewardday_sql = 'SELECT `reward_days` FROM ' . tablename('ewei_shop_task_poster') . ' WHERE  uniacid = :uniacid AND `id`=:id  AND poster_type=:poster_type ';
					$rewardday_data = array(':uniacid' => $_W['uniacid'], ':id' => $join_info['task_id'], ':poster_type' => $join_info['task_type']);
					$reward_days = pdo_fetchcolumn($rewardday_sql, $rewardday_data);
					if (0 < $reward_days) 
					{
						$reward_day = $createtime + $reward_days;
					}
					else 
					{
		...
		else if ($join_info['task_type'] == 2) 
		{
			...
			if (!(empty($rec_reward))) 
			{
				...
				if (isset($rec_reward[$rank]['goods'][$goods_id]) && !(empty($rec_reward[$rank]['goods'][$goods_id]))) 
				{
					...
					if (0 < $reward_days) 
					{
						$reward_day = $rec_reward[$rank]['reward_time'] + $reward_days;
					}
					else 
					{
