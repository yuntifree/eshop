...
class Rank_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$data = array( 'rank' => array('type' => intval($_GPC['type']), 'num' => intval($_GPC['num']), 'title' => trim($_GPC['title']), 'status' => intval($_GPC['status']), 'content' => $content) );
	...
	public function delete() 
	{
		...
		unset($list[$id]);
		$data = array( 'rank' => array('content' => $list) );
		m('common')->updatePluginset(array('commission' => $data));
